...
[![CI](https://github.com/mik-myp/contextweave/actions/workflows/ci.yml/badge.svg)](https://github.com/mik-myp/contextweave/actions/workflows/ci.yml) [![Latest release](https://img.shields.io/github/v/release/mik-myp/contextweave?label=release)](https://github.com/mik-myp/contextweave/releases)

## 项目状态

项目处于持续开发阶段。本节和下面的功能清单会随着实现、测试和发布结果更新；它们描述仓库当前可验证的能力，不代表尚未完成的路线图功能。

当前正式版本为 **v0.1.1**，对应标签 `v0.1.1`；历史正式版和 `v0.1.0-alpha.x` 标签保留。安装包通过 GitHub Actions 构建并发布到仓库 Releases。
...
- 日志查看：在「系统」分组中提供应用日志、运行会话和操作记录。应用日志支持级别、模块、方法、关键词、时间与耗时筛选，每 3 秒刷新，可暂停、跟随最新、查看/复制详情和清空；仅保留本次启动最近 1,000 条。历史会话和操作记录独立持久化，清空应用日志不影响历史。
...
1. 打开 **内核 → 安装内核**，选择 Fingerprint Chromium 的具体发行版本，下载并完成校验。当前安装入口只提供该内核的发行版本；不同版本可并存，环境保持绑定所选版本。
...
项目先完善可验证的个人使用闭环，再扩展自动化和团队能力。v0.1 系列对应当前实现范围，其余阶段是后续路线：

| 阶段      | 重点                                                       |
| --------- | ---------------------------------------------------------- |
| v0.1.0    | 指纹内核按版本安装、稳定环境身份、认证代理、运行闭环与恢复 |
| v0.1.1    | 应用日志、手动检查更新、列表与批量操作优化、系统导航整理   |
...
# 内部兼容回归：使用已知发行包验证保留的自定义源协议（无 UI 入口）
...
从 v0.1.1 起，在 **系统 → 系统设置 → 检查更新** 提供手动更新入口。点击后读取本仓库最新正式 Release，并按本机平台和架构选择 Windows 安装程序或 macOS DMG；支持下载进度、取消和 SHA-256 校验。离开页面后下载继续，返回可查看状态。草稿、alpha 等预发布版本不进入此入口。

下载完成后先停止浏览器环境，再点击「打开安装包」，退出 ContextWeave 后按系统提示安装。macOS 将应用替换到“应用程序”目录，Windows 使用安装程序完成更新。应用不会自动安装或重启；没有本机安装包或校验信息时会说明原因。

v0.1.0 安装包尚无此入口；使用该版本的用户首次需从 Releases 下载并安装 v0.1.1，之后可通过设置手动检查更新。

## 发布与许可

当前安装包不签名或公证；发布说明会提供支持平台、已知限制、SHA-256 和 SBOM。首次启动可能需要根据操作系统提示手动放行。
