...
- 环境：单项删除、按当前筛选选择并批量启动／停止／删除；逐项报告失败，删除记录时保留磁盘上的浏览器数据目录。
- 代理：列表搜索、多选筛选、排序、列设置、分页，新建／编辑 Dialog、单项和批量删除；检查环境引用与运行状态。密码由系统安全存储保护，可保留、替换或移除，不在列表和表单中回传。
- 内核：安装状态、版本、平台、可执行路径与能力查看；当前安装流程仍需补齐完整浏览器包的解压、资源文件和平台验收，不能视为成熟的内核分发系统。
- 运行记录：读取本地持久化浏览器会话，查看启动时间、状态和退出原因。
- 指纹能力：查看已注册内核的能力声明；这些声明尚不等于按内核版本完成的实测报告，专属指纹策略仍待真实提供方接入。
- 设置与关于：本地数据路径和凭据存储状态、版本平台信息及第三方许可声明；主题 Drawer 和界面语言统一从右上角调整。
