...
- 安装弹窗先选择内核类型，再获取该提供方的官方版本；首版只有 Fingerprint Chromium，可按注册入口扩展其他提供方。支持手动 HTTPS 下载链接：官方链接自动匹配可信元数据；自定义链接须指定兼容版本和 SHA-256，并确认来源与许可。自定义包按哈希使用独立安装 ID，不覆盖官方包或已有环境。
...
- 发布前核查 Windows x64、macOS x64/arm64 的管理端包和指纹内核运行支持差异；不支持的内核架构必须清楚标识。正式标签 `v0.1.0` 对应应用 `0.1.0`，Actions 成功后发布安装包及校验和。
...
- 本地质量门禁通过 119 项单元/行为测试。`pnpm test:desktop --require-native` 覆盖真实 Electron 沙盒、IPC、创建/重开、运行互斥、回收恢复；`pnpm test:fingerprint --require-provider` 已在 macOS arm64 完成官方 148.0.7778.215 发行包安装、真实启动、两次身份一致性、Cookie/localStorage、认证代理及回收恢复。用户授权的认证 SOCKS5 实测通过 HTTPS，代理测试与浏览器出口一致；测试凭据不进入仓库。
- 自定义 HTTPS 链接模式在 macOS arm64 完成相同的安装与运行回归，使用独立包 ID 与官方版本共存。UI 验收覆盖链接校验、下载进度、取消、更换链接重试、跨页面找回后台任务、代理表单与行操作测试、居中内容宽度、深色配色和设置分区独立滚动。打包内容解包后在隔离目录验证了主进程、沙盒桥接与新依赖加载。
