...
v0.1 的 A–F 工作包已落地：待验证提供方被阻止启动；提供 `scripts/verify-browser.mjs` 的可重复观测工具；服务拆分、配置修订、会话结果、操作日志、预检、按领域读取和回收恢复已接入 UI。普通环境仍是原生隔离模式，当前不会下载或分发尚未验证的指纹内核。
...
- 质量门禁包含格式、lint、类型检查和 100 项单元/行为测试。`pnpm test:desktop` 对真实 Electron 沙盒、IPC、创建/重开、重复启动与运行中删除拦截、会话记录、回收恢复执行端到端回归。CI 和构建矩阵已接入该脚本；无本机内核时明确报告未执行浏览器部分，不能记为内核运行通过。

2026-09-23 对代码提交 `97c80fe` 执行的[三平台构建与回归](https://github.com/mik-myp/contextweave/actions/runs/35863943295)全部通过；[Windows PR CI](https://github.com/mik-myp/contextweave/actions/runs/35863926102)也通过。三个平台日志均明确返回 `nativeLifecycle: passed`，没有跳过原生浏览器流程。

| 平台        | 质量门禁         | 真实桌面/原生环境回归 | 安装包与 SHA-256 |
| ----------- | ---------------- | --------------------- | ---------------- |
| Windows x64 | 通过，100 项测试 | 通过                  | 已生成并上传     |
| macOS x64   | 通过，100 项测试 | 通过                  | 已生成并上传     |
| macOS arm64 | 通过，100 项测试 | 通过                  | 已生成并上传     |

以上验证针对 CI runner 中探测到的原生浏览器和上述闭环，不等于指纹提供方资格验证，也未覆盖全部浏览器版本、真实账号或外部代理环境。正式签名、公证和新的 Release 尚未执行。
