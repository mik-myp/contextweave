...
A–F 已进入实现：待验证提供方被阻止启动；提供 `scripts/verify-browser.mjs` 的可重复观测工具；服务拆分、配置修订、会话结果、操作日志、预检、按领域读取和回收恢复已接入 UI。普通环境仍是原生隔离模式，当前不会下载或分发尚未验证的指纹内核。
...
- 质量门禁（100 项单元/行为测试）与 macOS arm64 安装包构建通过；`pnpm test:desktop --require-native` 对真实 Electron 沙盒、IPC、创建/重开、重复启动与运行中删除拦截、回收恢复执行端到端回归。CI 和构建矩阵已接入该脚本；无本机内核时明确报告未执行浏览器部分，不能记为内核运行通过。Windows/macOS x64 的真实浏览器流程及正式签名仍需对应平台验证；未将本地验证写成全平台发布完成。
