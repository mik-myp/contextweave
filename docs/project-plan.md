...
本方案替代此前按大量远期功能展开的路线。当前代码仍处于 v0.1 开发阶段；下面明确标为“目标”的能力尚未实现。重设计方案已进入 v0.1 实施；当前分支实现和验收范围见第 10.3 节，远期目标不作为已交付能力。当前能力以 [README](../README.md) 与工作区源码为准。
...
以下为调研基线与 v0.1 实施结果。开发分支通过的检查不等于全平台正式发布验收。

| 范围         | 当前依据                                         | 决定                                                                            |
| ------------ | ------------------------------------------------ | ------------------------------------------------------------------------------- |
| UI 基础      | 主题 Drawer、共享 DataTable、独立环境编辑页      | 已保留；新增预检、回收站、会话/操作分区，设置中展示遗留目录                     |
| 环境与凭据   | contracts、repository、独立用户目录、safeStorage | 已增加版本化配置与凭据损坏保护；代理认证只响应匹配的代理挑战                    |
| 指纹 Adapter | 原有 `--cw-*` 占位参数                           | 已移除并阻止未验证提供方启动；完整指纹接入归 v0.2                               |
| 能力描述     | 原来只有布尔声明                                 | 已拆分声明与证据状态；原生 CDP 实测绑定可执行文件身份和版本，其他项保持未验证   |
| 内核安装     | 旧安装器只支持单个可执行文件                     | v0.1 禁用未验证包安装；完整归档安装、资源和回滚随提供方验证落地                 |
| 配置与运行   | 原来一个 status 混合表达                         | 已增加不可变修订、active/trashed 生命周期、会话结果与独立 Operation             |
| 进程协调     | 原来集中在 Main                                  | 已拆为 application、命令协调、环境、内核、凭据、Worker 和 Runtime Supervisor    |
| 数据读取     | 原来跨领域周期刷新                               | 已采用按页面启用的 TanStack Query 与 IPC 失效事件，窗口重新聚焦读取活动页面快照 |
| 删除         | 原来删记录、留目录                               | 已实现回收站和同身份恢复；遗留目录只列出，不默认清理                            |
| 运行记录     | 原来缺结束时间和阶段                             | 已补实际配置/内核版本、结束时间、操作阶段及失败码                               |
| 功能入口     | 指纹能力独立导航                                 | 已移入内核详情并保留旧路由重定向；主题与语言仍在右上角                          |
...
│   ├── 代理             连接资源、引用情况、检测结果
│   └── 内核             发行版本安装；后续扩展为浏览器资源
└── 系统
    ├── 系统设置         数据存储、检查更新、关于；后续扩展运行与连接设置
    └── 日志查看         应用日志、运行会话、操作记录
...
系统设置保持页内左右结构，全局菜单不展开其子菜单。设置项包含本地存储、检查更新和关于；窄窗口使用页内分区切换。外观与界面语言仅在右上角提供。
...
- 页码提供首尾与相邻页、必要省略号及跳页入口，避免数千页同时渲染。列表占满当前页面的可用高度，分页保持在底部靠右；数据较少时不紧贴表格，数据较多时表格区域独立滚动并固定表头。
- 勾选后的批量操作栏悬浮于列表内容区域底部、分页上方，不参与文档流，不因显示/隐藏挤动表格或分页。位置随侧栏、全宽/居中和方向设置调整；悬浮栏之外仍可操作与滚动表格。操作采用图标按钮与 Tooltip，提示展示操作名称及可操作数量；已选数量保持可见，清除选择也提供 Tooltip。
...
**日志查看：** 归入「系统」分组，应用日志、浏览器会话与后台操作用页签区分。应用日志的范围和交互见 4.7；历史记录独立持久化。记录开始/结束时间、使用的配置版本、失败阶段、退出原因及恢复动作。常规界面不暴露 PID、原始 CDP 端口、代理密码或完整启动参数。内核、能力比较、日志查看继续不显示冗余页首标题和说明。
...
- 操作列按当前记录的实际操作去重；前三项使用图标按钮和 Tooltip，第四项起放入省略号菜单，菜单使用图标和文字。普通按钮使用主题强调色及浅色背景，危险操作保持危险色；禁用原因、运行锁、确认与批量反馈保持有效。
- 内核安装入口仅展示 Fingerprint Chromium 的项目发行版本，直接选版本后下载。不展示项目来源切换、提供方选择或自定义链接表单；底层的历史包读取和自定义来源验收能力保留，不作为当前产品入口。
- 侧边栏分为「工作台」和「系统」：前者放环境、代理与内核，后者依次放日志查看、系统设置。系统设置固定为系统分组的最后一项，后续新增系统菜单均排在它之前；系统设置不展开子菜单。
- 日志查看默认展示应用日志，保留运行会话、操作记录两个历史页签。参考 Ant Browser 的级别、组件、方法、关键词、时间与耗时筛选，提供自动刷新/暂停、跟随最新日志、刷新、详情、复制和确认清空。
- 应用日志来自 Main 的实际命令与生命周期事件，采用最多 1000 条的当前会话缓冲区，明确显示保留范围与淘汰数量。只记录事件、方法、状态、错误码、耗时及受控资源 ID，不记录任意请求/响应、代理密码、Cookie 或凭据。清空只处理当前会话缓冲区；历史运行和操作记录仍保存在数据库。
- 设置提供手动更新：仅用户点击时查询本仓库最新正式 Release，以语义版本比较并按本机平台/架构选包。流程为检查 → 下载 → SHA-256 校验 → 手动打开安装包；支持进度、取消、失败重试、发行说明及离开页面后继续下载。安装前由用户停止环境并退出应用，不自动中断浏览器或替换应用。
- 当前未签名发行包使用系统安装器完成更新；签名、公证和更新元数据齐备后再评估自动安装。无本机安装包、缺少可信哈希、网络失败分别说明，不能显示为「已是最新版本」。
...
浏览器控制连接采用 30 秒的启动等待窗口，以容纳新目录和繁忙机器的冷启动；等待期间响应显式取消，检测到子进程提前退出时终止等待并报告启动失败。超过等待窗口返回 `CONTROL_TIMEOUT`，由同一运行协调器回收拥有的进程和锁，不自动再启动一个浏览器。
...
| 阶段                 | 主要交付                                                                       | 完成标准                                                                                            |
| -------------------- | ------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------- |
| v0.1：指纹浏览器闭环 | 官方指纹内核下载与校验、稳定身份、环境生命周期、认证代理和连接测试、桌面安装包 | 下载内核 → 创建环境与代理 → 启动访问 → 停止重开保留身份与数据；Windows/macOS 受支持组合完成实际验收 |
| v0.2：组织与能力扩展 | 分组/标签、配置复制、模板、更多指纹选项与网络诊断                              | 多次重启身份稳定，关键上下文一致；不支持字段不静默生效；复制和代理变更语义正确；真实平台矩阵        |
...
### 10.0 v0.1 范围调整（2026-09-23）

维护者明确要求 v0.1 可实际完整使用指纹浏览器，因此将首个真实指纹提供方和代理连接检测提前交付。此前“仅原生浏览器闭环”验收不再代表新版 v0.1 完成。

- 首个提供方为 `adryfish/fingerprint-chromium`，安装入口从官方 Releases 选择具体版本，按版本锁定资源大小与 GitHub 公布的 SHA-256；`148.0.7778.215` 作为已核查的默认推荐和离线目录兜底。Windows 使用便携 ZIP；macOS 使用 DMG，按实际 Mach-O 架构确定可用平台。下载按需进行，不随管理端捆绑浏览器。
- 安装弹窗只展示 Fingerprint Chromium 发行版本、包信息与安装状态，暂不提供类型选择或自定义下载链接。提供方注册表与已验证的自定义源底层协议保留以兼容历史安装和回归测试；扩展其他内核或恢复自定义源入口时再独立评估。
- 内核列表只展示已安装的具体版本及已发现的本机浏览器；独立“安装内核”Dialog 选择发行版本，不显示待安装占位行。多个版本并存，环境绑定具体版本，不随推荐版本变化自动切换。无适配支持、无官方哈希或无对应平台包的版本不能安装。
- 安装过程具有下载进度、显式取消、大小/哈希检查、受控解包、可执行文件检查和原子提交；失败不改变已安装内核。保留上游许可证及第三方资源，不把仓库 BSD 许可解释为所有 Chromium 组件的许可。
- 每个指纹环境在 Main 生成并保存一次 32 位身份种子，固定内核版本和独立用户目录。只开放上游实际支持且经测试的参数，不使用旧占位 canvas/audio/webgl 参数。
- 认证 SOCKS5 经每会话独立的 loopback HTTP 桥接转发；随机本地凭据、远端 DNS、禁止直连降级，随失败/停止/退出清理。新增/编辑/列表测试使用同一转发路径，验证实际 HTTPS 请求而非仅 TCP 端口开放。
- 配置详情遵循全局主题内容宽度；列表状态使用语义颜色；设置取消总标题，左右独立滚动；关于归入设置。
- 发布前核查 Windows x64、macOS x64/arm64 的管理端包和指纹内核运行支持差异；不支持的内核架构必须清楚标识。正式标签必须与应用版本一致；本次 `v0.1.1` 对应应用 `0.1.1`，Actions 成功后发布安装包及校验和。
...
### 10.3 当前实现与验收（2026-09-24）

A–F 和 10.0 扩充范围已实现：已接入官方指纹发行版本目录与安装流程，支持并存版本和环境固定身份；服务拆分、配置修订、会话结果、操作日志、预检、按领域读取和回收恢复已接入 UI。Standard Chromium 保持原生隔离模式，不增加指纹能力声明。

- 元数据采用 `PRAGMA user_version` 和事务迁移。旧数据库迁移前使用 `VACUUM INTO` 保存一致性备份，失败回滚；配置原文成为修订 1，不改环境 ID、代理引用和浏览器目录。旧会话未知的结束时间保持未知，不编造历史。
- 回收站恢复保留环境身份、修订及目录；永久清理和完整快照仍属于后续阶段。回收站中的代理引用同样参与删除保护。
- 运行恢复不依据保存的 PID 自动杀进程。旧进程仍活着时要求先关闭浏览器；无效/不可读的锁不自动夺取。确认进程退出后才能释放遗留状态。
- Main 统一验证 IPC 来源；Preload 打包包含边界 schema 所需的 Zod，保持 sandbox 和 contextIsolation。Worker 的旧 smoke 入口限制为 HTTP(S) 测试，不接受 Renderer 提供的任意截图路径。
- 本机验收覆盖创建、修订冲突、预检、启动/停止、回收/恢复及页面切换；自动化覆盖迁移、配置修订、运行竞争与取消、凭据损坏、代理认证隔离和草稿恢复。
- 本机 Chrome 153.0.8010.53（macOS arm64）通过回环测试页面的两次 headless 启动观测，参数一致、页面存储保留。这个结果不外推到其他内核、操作系统、有头/无头一致性或反检测效果。
- 本地质量门禁通过 153 项单元/行为测试。`pnpm test:desktop --require-native` 覆盖真实 Electron 沙盒、IPC、创建/重开、运行互斥、回收恢复；`pnpm test:fingerprint --require-provider` 已在 macOS arm64 完成官方 148.0.7778.215 发行包安装、真实启动、两次身份一致性、Cookie/localStorage、认证代理及回收恢复。用户授权的认证 SOCKS5 实测通过 HTTPS，代理测试与浏览器出口一致；测试凭据不进入仓库。
- 底层自定义 HTTPS 源在 macOS arm64 保留已有安装与运行回归证据，使用独立包 ID 与官方版本共存；当前安装 UI 已按 4.7 收敛为 Fingerprint Chromium 发行版本。下载进度、取消、后台任务、代理表单与行操作、居中宽度、深色配色和设置分区独立滚动已有 UI 验收；打包内容解包后在隔离目录验证了主进程、沙盒桥接与新依赖加载。
- 系统工具增量通过真实 Electron 验收：应用日志记录实际环境命令与状态，验证自动刷新、暂停、关键词筛选、1,000 条上限及淘汰计数、向上滚动停止跟随、复制、清空后历史保留，以及深色强调色和居中布局。手动更新完成正式版本在线检查，并已在隔离的旧版本测试壳中验证真实安装包下载与 SHA-256；系统替换安装仍由用户执行。
- 发布矩阵包含 Windows x64、macOS x64、macOS arm64；Windows x64 与 macOS arm64 强制执行真实指纹内核回归，Intel Mac 只验证管理端与本机浏览器路径。签名/公证尚未配置；构建、浏览器运行与签名状态分别记录，不能互相替代。
- 三平台流水线 [Build desktop #35885047776](https://github.com/mik-myp/contextweave/actions/runs/35885047776) 与 [CI #35885047786](https://github.com/mik-myp/contextweave/actions/runs/35885047786) 已通过。正式发布仍由标签工作流针对最终提交重新执行检查、运行验收和打包，全部成功后上传安装包、SHA-256 与 SBOM。

回退数据库迁移时，先退出客户端与相关浏览器，保留当前数据库、WAL/SHM 和浏览器目录，再使用迁移旁路备份恢复到兼容的旧客户端。不能在运行中覆盖数据库，也不能用旧客户端打开新 schema；回退后的新修订不会自动带回旧库。
...
- **发布：** 保持 ContextWeave 自身许可状态透明，不擅自为维护者选择代码许可证；审查复用代码与内核发行材料。构建哈希、来源与第三方许可随版本交付；不要把一个上游应用的许可证用于它捆绑的全部二进制。
...
### Ant Browser 链接下载复核（2026-09-24）

补充核查提交 `61feab721f71d65301dfa45397cb9e15e28e8fd8` 的 [CoreDownloadModal](https://github.com/black-ant/Ant-Browser/blob/61feab721f71d65301dfa45397cb9e15e28e8fd8/frontend/src/modules/browser/pages/coreManagement/CoreDownloadModal.tsx)、[下载任务](https://github.com/black-ant/Ant-Browser/blob/61feab721f71d65301dfa45397cb9e15e28e8fd8/backend/internal/browser/download_core_task.go)、[传输](https://github.com/black-ant/Ant-Browser/blob/61feab721f71d65301dfa45397cb9e15e28e8fd8/backend/internal/browser/download_core_transfer.go) 与 [解压](https://github.com/black-ant/Ant-Browser/blob/61feab721f71d65301dfa45397cb9e15e28e8fd8/backend/internal/browser/download_core_extract.go) 实现：表单要求名称和地址，允许选择下载代理；该路径没有要求用户提供四段版本号或 SHA-256，也没有 ContextWeave 的适配版本清单限制。实现仍包含同名目录检查、解压路径检查和 `ValidateCorePath`，不能视为完全不做校验。ContextWeave 当前安装入口仅提供 Fingerprint Chromium 项目发行版本，继续保留可信哈希、平台/架构及适配能力验证。
