...
安装源以提供方注册表组织，UI 的类型选择与版本查询使用提供方 ID。官方目录缓存与已安装清单分离；自定义 HTTPS 源经用户确认、版本适配和 SHA-256 校验，使用独立包 ID，与官方同版本并存。下载限量、逐跳检查 HTTPS，临时 URL 查询参数不进入持久化来源记录；实际启动时核对 CDP 返回的浏览器版本与绑定版本，失败即停止。
