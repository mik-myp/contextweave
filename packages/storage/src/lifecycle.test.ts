...
it('upgrades v1 proxy names without changing existing credentials, IDs or environment revisions', () => {
  ...
  previous.sqlite.exec('DROP TABLE credential_cleanup; ALTER TABLE proxies DROP COLUMN name; PRAGMA user_version = 1;')
  previous.close()
  ...
  expect(readdirSync(root).some((name) => name.includes('.before-v3-'))).toBe(true)
})
